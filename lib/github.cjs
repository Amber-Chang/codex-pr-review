...
function postReviewComments(ref, cwd, comments) {
  ensureGhAuth(cwd);
  ...
  const pr = ghJson(["api", `repos/${ref.owner}/${ref.repo}/pulls/${ref.number}`], cwd);
  ...
  const reviewComments = anchored.map((comment) => {
    return ghJson(
      [
        "api",
        "--method",
  ...
  if (unanchored.length > 0) {
    const body = [
      ...
      "",
      ...unanchored.map((comment, index) => `${index + 1}. ${comment.body}`),
    ].join("\n");
    issueComment = ghJson(
      [
        "api",
